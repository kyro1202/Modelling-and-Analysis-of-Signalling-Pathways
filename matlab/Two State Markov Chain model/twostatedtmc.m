...
while(tran_a_to_b < 3 || tran_b_to_a < 3)
    chain_length = 2136;            % length of the simulation
    ...
    for i=2:chain_length
        this_step_distribution = stp(previous_state,:);
        ...
        % updating the total transitions
...
while(total_tran < N+M)
    ...
    while(total_tran < N+M)
		% dtmc (Discrete Time Markov Chain) simulation
		this_step_distribution = stp(previous_state,:);
        cumulative_distribution = cumsum(this_step_distribution);
        r = rand();
        current_state = find(cumulative_distribution>r,1);
